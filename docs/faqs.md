...
A password is still required for authentication by default, even if you have set up an SSH key. This is because the system is configured to require two-factor authentication (2FA) for all users. When you log in, you will be prompted to enter your MIT Kerberos password and respond to a Duo push notification on your phone. This is an important security measure to protect your account and the cluster. However, you can reduce the number of times you have to do 2FA by logging into [ORCD OnDemand](https://orcd-ood.mit.edu) periodically.
